...
INSERT INTO score_report(id, name, subject, score) VALUES 
	(1, '张三', '语文', 80),
    (2, '李四', '语文', 90),
    (3, '王五', '语文', 60),
    (4, '王胖子', '数学', 59),
...
SELECT id, name, subject, score 
FROM score_report 
WHERE subject = '语文' ORDER BY score DESC;

-- 	查找科目为语文的成绩最高的学生。
SELECT id, name, subject, score 
FROM score_report 
WHERE subject = '语文' ORDER BY score DESC LIMIT 1;

-- 	查找姓郭的学生中数学学得最好的一位。
SELECT id, name, subject, score 
FROM score_report 
WHERE name LIKE '郭%' AND subject = '数学' 
ORDER BY score DESC 
LIMIT 1;
